...
void __fastcall CAvatar__SetChairHeight_hook(CAvatar* pThis, void* _EDX, int nItemID) {
    ...
    // Pack offset into m_nChairHeight field
    int packed = 0xF0000000;
    packed |= (x & 0x0FFF) << 16;
    packed |= (y & 0xFFFF);
    pThis->m_nChairHeight() = packed;
}
...
void __fastcall CAvatar__Update_hook(CAvatar* pThis, void* _EDX) {
    ...
    int packed = pThis->m_nChairHeight();
    if (packed & 0xF0000000) {
        ...
        CHECK_HRESULT(pThis->m_pBodyOrigin()->raw_RelMove(x, y, vEmpty, vEmpty));
    }
}
